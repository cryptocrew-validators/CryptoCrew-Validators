...
function generateMDTable(relayers, chains) {
  ...
  for (const chain of chains.chains) {
    ...
    if (channels) {
      const mdContent = generateMDContent(channels);
...
function generateMDContent(channels) {
  const header = '## CryptoCrew IBC relayer\n\n| src_chain | dst_chain | IBC port | IBC channel |\n| --------------- | --------------- | ------------ | -------------- |\n';
  const rows = channels.map(channel => `| ${channel.chain_id} | ${channel.dst_chain_id} | ${channel.port_id} | ${channel.channel_id} |`).join('\n');
